...
function setupCards() {
  const cards = document.querySelectorAll('.card[data-delay-max-ms]');
  cards.forEach(card => {
    card.addEventListener('click', async (e) => {
      e.preventDefault();

      const href = card.getAttribute('href') || 'https://redjgames.base44.app';
      const max = Number(card.dataset.delayMaxMs || 2000);
...
function setupMobileBounce() {
  ...
  window.addEventListener('scroll', () => {
    ...
    scrollEndTimeout = setTimeout(() => {
      ...
      // Prüfen ob wir ganz unten sind (mit 50px Toleranz)
      if (scrollTop + windowHeight >= documentHeight - 50) {
        page.classList.add('scrolled-to-bottom');
        
        // Bounce-Effekt nach kurzer Zeit wieder entfernen
        setTimeout(() => {
          page.classList.remove('scrolled-to-bottom');
        }, 300);
      }
    }, 100);
  });
